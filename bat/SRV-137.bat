...
powershell -Command "& { 
    $rules = Get-NetFirewallRule | Where-Object { $_.Enabled -eq 'True' -and $_.Action -eq 'Allow' }
    if ($rules) {
        echo '방화벽 규칙: ' >> '%TMP1%'
        $rules | Format-Table Name, Action, Direction, Enabled -AutoSize | Out-String -Width 512 >> '%TMP1%'
    } else {
        echo '적절한 접근 제한 규칙이 설정되지 않았습니다.' >> '%TMP1%'
    }
}"
